...
export const Lessons = () => {
  ...
  const combinedCourses = useMemo(() => {
    return [...apiCourses, ...allCourses];
  }, [apiCourses]);

  const filteredCourses = useMemo(() => {
    let result = combinedCourses;
    if (activeSubject !== 'Tất cả') {
      result = result.filter(course => course.subject === activeSubject);
    }
    if (searchQuery) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
                  ...
                  {subjectsFilter.map(subj => (
                    <button 
                      ...
                      className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                        activeSubject === subj 
                          ? 'border border-surface-raised text-surface-raised bg-blue-50' 
                          : 'border border-border-default text-text-tertiary hover:bg-surface-strong'
                      }`}
                    >
                      {subj}
                    </button>
          ...
          <main className="w-full lg:w-3/4 flex flex-col">
            <div className="mb-4 text-text-tertiary text-sm">
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
                  {currentCourses.map((course) => (
                    <div 
                      key={course.id} onClick={() => navigate(`/lessons/${course.id}`)}
                    ...
                    >
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        ...
                        <h3 className="font-bold text-text-primary text-lg leading-snug mb-4 flex-1 line-clamp-2 group-hover:text-surface-raised transition-colors">{course.title}</h3>
                ...
                {totalPages > 1 && (
                  <div className="flex justify-center items-center gap-2 mt-auto pt-4">
                    <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="p-2 rounded-lg border border-border-default text-text-tertiary hover:bg-surface-strong disabled:opacity-50 disabled:cursor-not-allowed">
                      <ChevronLeft size={20} />
                    </button>
                    {Array.from({ length: totalPages }).map((_, idx) => (
                      <button
                        key={idx + 1} onClick={() => handlePageChange(idx + 1)}
                        className={`w-10 h-10 rounded-lg text-sm font-medium transition-colors ${ currentPage === idx + 1 ? 'bg-surface-raised text-white' : 'border border-border-default text-text-tertiary hover:bg-surface-strong' }`}
                      >
                        {idx + 1}
                      </button>
                    ))}
                    <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-2 rounded-lg border border-border-default text-text-tertiary hover:bg-surface-strong disabled:opacity-50 disabled:cursor-not-allowed">
