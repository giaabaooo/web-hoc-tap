...
import { Plus, Image as ImageIcon, Video, Trash2, FileEdit, Mic, Headphones, ScanLine, Type, ArrowLeft, FileText } from 'lucide-react';
...
export const EditCourse = () => {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const data = res.data.course; // ĐÃ FIX LỖI DATA TẠI ĐÂY
        
        setCourseInfo({
          title: data.title || '', description: data.description || '', subject: data.subject || 'Tiếng Anh', tag: data.tag || 'Cơ bản', price: data.price || 0, thumbnail: data.thumbnail || ''
        });
  ...
  const LessonPreview = ({ lesson }) => {
    ...
    if (lesson.type === 'youtube') {
      ...
      if (yId) return <iframe className="w-full h-full" src={`https://www.youtube.com/embed/${yId}`} title="YouTube preview" frameBorder="0" allowFullScreen></iframe>;
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-6 relative overflow-hidden">
          ...
          <div className="mt-6">
            ...
            <div className="mb-4">
              ...
              <textarea value={exercise.instruction || ''} onChange={(e) => updateExercise(cIndex, sIndex, lIndex, eIndex, 'instruction', e.target.value)} placeholder="VD: Lắng nghe đoạn audio và chọn đáp án đúng..." className="w-full p-2.5 text-sm bg-yellow-50 border border-yellow-200 rounded-lg outline-none focus:border-yellow-400 focus:bg-yellow-100/50 transition-colors resize-none" rows="2" />
            </div>
